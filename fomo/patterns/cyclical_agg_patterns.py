...
def missing_data_probabilities_new(flagged_df, time_column, axes, basis_rate=15, missingness_interval=15, missing_flags_column='Missing_Flag'):
    ...
    if missing_flags_column not in flagged_df.columns:
    ...
    if missingness_interval < basis_rate:
        raise Exception("Resampling Error: missingness_interval must be greater than or equal to the basis_rate.")
    
     # Convert Missing_Flag to integer to calculate percentages later
    flagged_df['Missing_Flag'] = flagged_df['Missing_Flag'].astype(int)

    # Create matrix where we take all columns but the datetime column
    matrix = flagged_df
    ...
    num_entries = missingness_matrix_week[missing_flags_column].count() # gets count of non-nan entries
    ...
    if isinstance(axes, str) or len(axes) == 1:
        ...
        if axes == ['day', 'week'] or axes == ['week', 'day']:
            ...
            unflattened = missingness_matrix_week.groupby(['Day','Time'])['Missing_Fraction'].agg(['mean','var']).reset_index()
            missingness_avg = unflattened.pivot(index='Day', columns='Time', values='mean').loc[['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday']]
            missingness_var = unflattened.pivot(index='Day', columns='Time', values='var').loc[['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday']]
...
##### Usage
data_matrix = np.zeros((len(all_files),24))

def get_missingness_avg(file):
    person_df = pd.read_parquet(file, engine='pyarrow').reset_index()
    missingness_avg, missingness_var = missing_data_probabilities_new(person_df, time_column='index', axes=['day'], missingness_interval=60)
    
    # Flatten the data matrix
    missingness_avg = missingness_avg.flatten()
    return missingness_avg

avgs = p_map(get_missingness_avg, all_files)
data_matrix = np.array(avgs)
np.save('24h_data_2year.npy', data_matrix)


data_matrix = np.load('24h_data_2year.npy')
scaled_matrix = StandardScaler(with_std=False).fit_transform(data_matrix.T).T
double_scaled_matrix = StandardScaler(with_std=True).fit_transform(data_matrix.T).T


###### version in week level
import numpy as np
import pandas as pd
